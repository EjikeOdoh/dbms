...
export class VolunteerParticipationService {
  ...
  async create(
    createVolunteerParticipationDto: CreateVolunteerParticipationDto,
  ) {
    const { volunteerId, programId, ...rest } = createVolunteerParticipationDto;
    const volunteer = await this.vRepository.findOne({
  ...
  async update(
    id: number,
    updateVolunteerParticipationDto: UpdateVolunteerParticipationDto,
  ) {
